    ...
    def build(self):
        ...
        for center in self.nodes:
            ...
            for e1, e2, e3 in combinations(incident, 3):
                ...
                for n in [cid, a, b, c_val]:
                    color = self.color.get(n, "unknown")
                    if color in GROUPS:
                        counts[color] += 1
        ...
        if self.fairness_terms:
            for g in GROUPS:
                ...
                self.add_bound(f"total_{g}_eps >= 0")
            ...
            # Total per color
            for g in GROUPS:
                terms = [f"{counts[g]} {var}" for var, counts in self.fairness_terms]
                self.add_constraint(f"total_{g}_eps - {' - '.join(terms)} = 0")
            
            # Total eps
            total_terms = [f"total_{g}_eps" for g in GROUPS]
            self.add_constraint(f"total_eps - {' - '.join(total_terms)} = 0")
            
            # Imbalance example: max-min among groups
            self.add_constraint(f"imbalance - total_red_eps + total_blue_eps >= 0")
            self.add_constraint(f"imbalance - total_blue_eps + total_red_eps >= 0")
            self.add_constraint(f"imbalance - total_red_eps + total_green_eps >= 0")
            self.add_constraint(f"imbalance - total_green_eps + total_red_eps >= 0")
            self.add_constraint(f"imbalance - total_blue_eps + total_green_eps >= 0")
            self.add_constraint(f"imbalance - total_green_eps + total_blue_eps >= 0")
    
    # --- Output LP ---
    def to_lp(self):
        lines = ["Maximize"]
        obj_terms = [f"{LAMBDA_1} {var}" for var in sorted(self.c_vars)]
        obj_line = "  " + (" + ".join(obj_terms) + f" - {LAMBDA_2} imbalance" if obj_terms else f"0 - {LAMBDA_2} imbalance")
